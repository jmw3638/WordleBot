...
class WordGuesser:
    ...
    def read_in_results(self, word: str, results: list):
        ...
        if len(word) != len(results):
            util.vlog(
                "Word length {} does not match results length {}".format(
                    len(word), len(results)
                )
            )
            return False
    ...
    def get_possible_words(self):
        ...
        for i in range(self.word_length):
            ...
            if right_l != "":
                util.vlog(
                    "Letter '{}' in right set at {}".format(right_l, i), 6
                )
                reg_exp = right_l
            # Remove letters in partial set for current position
            elif partial_l != "":
                util.vlog(
                    "Letters '{}' in partial set at {}".format(partial_l, i), 6
                )
                reg_exp = "[^"
                reg_exp += partial_l
                reg_exp += wrong_l
...
class WordSet:
    ...
    def add_letter(self, letter: str, position: int):
        ...
        if self.multiple and letter in self.letters[position]:
            util.vlog(
                "Letter '{}' already in word set at position {}".format(
                    letter, position
                ),
                5,
            )
        ...
        if not self.multiple and self.letters[position] != "":
            util.vlog(
                "Failed to add '{}' to word set, position {} contains '{}'".format(
                    letter, position, self.letters[position]
                ),
                5,
            )
            return False
        ...
        util.vlog(
            "Adding '{}' to word set (position: {})".format(letter, position), 5
        )
    ...
    def get_pos_letters(self, position: int):
        if position < 0 or position > len(self.letters):
            util.vlog(
                "Invalid position {} for word set of length {}".format(
                    position, len(self.letters)
                )
            )
            return False
    ...
    def letter_in_set(self, letter: str):
        ...
        for i in range(len(self.letters)):
            if self.letter_at_position(letter, i):
                util.vlog(
                    "Letter '{}' in word set at position {}".format(letter, i)
                )
                contains = True
        return contains
