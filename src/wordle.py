...
def parse_args(parser: argparse.ArgumentParser) -> argparse.Namespace:
    ...
    bot_parser = subparsers.add_parser("bot", help="run Wordle browser bot")

    sim_parser = subparsers.add_parser("sim", help="run Wordle simulation")

    # General argument parser arguments

    parser.add_argument("dictionary", help="word dictionary file")

    parser.add_argument(
        "-w", "--allowed-words", help="allowed words dictionary file"
    )

    parser.add_argument(
        "-v", "--verbose", help="use verbose logging", action="count", default=0
    )
...
def browser_bot(args: argparse.Namespace, words_list: str) -> bool:
    ...
    if not board_tl_coords:
        print(
            "Error: invalid board (top left) coords '{}'".format(
                args.board_coords
            )
        )
        return False
...
def simulation(
    args: argparse.Namespace, words_list: str, allowed_words: list = None
) -> bool:
    ...
    if args.first:
        if len(args.first) != 5:
            print(
                "Error: invalid word length of {} '{}'".format(
                    len(args.first), args.first
                )
            )
            return False
    ...
    results = sim.run_simulation(
        args.iterations, start_word, goal_word, allowed_words
    )
